...
class SelfAttention(pl.LightningModule):
    ...
    def forward(self, value, key, query, mask=None):
        ...
        # if the mask is applied, fills with the -infinity
        # value all the element in the K matrix with a mask==0
        if mask is not None:
            energy = energy.masked_fill(mask == 0, float("-1e10"))
        attention = self.dropout(
            F.softmax(energy / self.dot_scale.to(key.device), dim=-1))
        ...
        # out = [batch_size, src_seq_len, n_heads * head_dim]
        # or
        # out = [batch_size, src_seq_len, embedding_size]

        out = self.fc_out(out)
        # Out = [batch_size, seq_size, embedding_dim]
        return out
...
class Encoder (pl.LightningModule):
    def __init__(self, vocab_size, embedding_dim, num_trans_block,
                 num_heads, dropout, max_input_len):
    ...
    def forward(self, input, mask):
        ...
        # Sum the position encoding + the token encoding
        # to get the positional encoding
...
class Decoder (pl.LightningModule):
    def __init__(self, vocab_size, embedding_dim, num_heads,
                 num_transformer_block, dropout, max_input_len):
    ...
    def forward(self, input, enc_out, src_mask, trg_mask):
        ...
        # Sum the position encoding + the token
        # encoding to get the positional encoding
...
class TpTransformer(pl.LightningModule):
    def __init__(
        ...
        self.encoder = Encoder(vocab_size, embed_size,
                               num_layers, heads, dropout, max_input_len)
        self.decoder = Decoder(vocab_size, embed_size,
                               heads, num_layers, dropout, max_input_len)
    ...
    def make_trg_mask(self, trg):
        ...
        trg_sub_mask = torch.tril(
            torch.ones((trg_len, trg_len), dtype=torch.uint8,
                       device=trg.device))

        trg_mask = trg_pad_mask.type(
            torch.ByteTensor) & trg_sub_mask.type(torch.ByteTensor)
    ...
    def crossEntropyLoss(self, logits, labels):
        return nn.CrossEntropyLoss(ignore_index=0)(logits.reshape(-1, 73),
