...
class Multiclass(pl.LightningModule):
    ...
    def add_padding(self, data, max_lenght):
        ...
        for i in data:
            ...
            result.append(
                (torch.cat((i.to(self.device), torch.zeros(
                    to_add).to(self.device)))))
    ...
    def nllloss(self, logits, labels):
        ...
        return nn.NLLLoss(ignore_index=0)(logits.reshape(
            -1, 73), torch.flatten(labels).long())
    ...
    def test_step(self, test_batch, other):
        ...
        accuracy = torch.sum(
            test_batch[1] == prediction_rm_pad).item() / (len(y) * 1.0)
        output = dict({
            'test_loss': loss,
